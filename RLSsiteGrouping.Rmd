...
#Next I want to assing group membership to sites based on their PC score. The first 2 PC axes more or less define 4 groups 

siteData2 = merge(x = siteData1, y = pscores, by.x = "SiteCode", by.y = "row.names", all.x = TRUE) 

sitesWithGroups <- siteData2 %>% mutate(g = case_when((siteData2$PC1 < 0 & siteData2$PC2 < 0) ~ 1, siteData2$PC1 > 4 ~  2, ((siteData2$PC1 > 1 & siteData2$PC1 < 3) & siteData2$PC2 < 1) ~  3, (siteData2$PC1 < 0 & siteData2$PC2 >0) ~  4)) # this could be done better, because I think case_when only works with two conditions?? 

sitesWithGroups$g[is.na(sitesWithGroups$g)] <- 5 # assign other sites a code of 7. NOTE, these codes are just arbitrary to produce better colours in the plot below, this should be improved. One problem with this approach is that it assigns code to both intermediate group in PCA and also to sites with NA values which were excluded from the analyses
...
#  filter (Location != "Port Phillip Bay" & Location != "Port Phillip Heads" & Location != "Ninepin External") %>%
  #    filter(Method == 1) %>%
        #  filter (year > 2005) %>%
...
#PCAdataMain is the main data list by siteCode with environmental variables and  abundances of 53 species. The PCAdataMain is the main data file with environmental and species abundance data for 531 sites 

## Then we can do PCA on the sites 
...
## Or alternatively try k-means clustering on locationxspecies data. This is not a very intersting result, because just a few locations are separated and most others are grouped into 1

clus <- kmeans(locationPCA, 3, nstart = 20)
