...
1) Develop a protocol that to identify species and locations to be included in regional models size spectrum models. We want to have a somewhat automated procedure to identify key groups important for models. For that we need to assess the spatial scale over which locations can be pooled and one model applied. Next, we need to assess which species should be included, based on the frequency of occurrence, abundance or other factors defining ecological importance (invasive species). Initially we plan to have a model at the level of ecoregion and start with the Bassian ecoregion. However, if locations in one ecoregions are too different based on their species composition we need to split them into several models. 

2) Develop a protocol to extract all available information about species from publicly available databases (fishbased, Thorson et al. 2017 life-history parameter paper, Rick's thermal optima and any other data) and group this information to provide key parameters required for size spectrum models. 
...
# there are 470 sites. Too many! We probably do not need them, just focus on Locations
...
locVert <- rls_bas %>% 
    filter (( PHYLUM == "Chordata" & GENUS != "NA" & CLASS != "Ascidiacea") | CLASS == "Cephalopoda") %>% 
           group_by(Location) %>% 
           summarise (latt = mean(SiteLat), long = mean(SiteLong), nsp = n_distinct(SpeciesID), year = n_distinct(year), dep_min = min(Depth), dep_max = max(Depth)) %>%
      arrange(desc(latt)) %>%
        rename("Mean lattitude" = latt, "Mean longitude" = long, "No vertebrate sp" = nsp, "No years sampled" = year, "Min depth" = dep_min, "Max_depth" = dep_max)
...
mainVertOccur <- rls_bas  %>% 
  filter (( PHYLUM == "Chordata" & GENUS != "NA" & CLASS != "Ascidiacea") | CLASS == "Cephalopoda") %>% 
      ...
      summarise (years = n_distinct(year), years2000 = n_distinct(ye2000), years2010 = n_distinct(ye2010), dep_min = min(Depth), dep_max = max(Depth), sizeMax = max(SizeClass), latt = round(mean(SiteLat),3), lattMin = round(min(SiteLat),3), lattMax = round(max(SiteLat),3), lattRange = round((max(SiteLat) - min(SiteLat)),3), long = round(mean(SiteLong),3)) %>%
        filter(years > 3) %>% #if years > 4 we have 122 species
        arrange(desc(years))

MainVertListOc <- mainVertOccur[[1]] ## list of vertebrate species filtered by occurrence in years

#Second calculate mean abundance and biomass of these species per survey and filter out rare species 
...
MainVertList <- VertPerSurvey[[1]] ## final list of common vertebrates with 3 filters applied = 86 vertebrates

#RQ3 = what are reasoable criteria? Is surveyID the right field to summarise survey data

#then get all sorts of details for these species 
mainVertDetails <- rls_bas  %>% 
  filter (TAXONOMIC_NAME %in% MainVertList) %>% 
      group_by (TAXONOMIC_NAME, GENUS, CLASS, FAMILY, ORDER) %>%
      summarise (years = n_distinct(year), years2000 = n_distinct(ye2000), years2010 = n_distinct(ye2010), dep_min = min(Depth), dep_max = max(Depth), latt = round(mean(SiteLat),3), lattMin = round(min(SiteLat),3), lattMax = round(max(SiteLat),3), lattRange = round((max(SiteLat) - min(SiteLat)),3), long = round(mean(SiteLong),3)) 
...
Table above shows that Port Phillip (VIC) has a lot more species, and that thre are five main groups of locations with longer term sampling: Ninepin, Tinderbox, Maria, Bicheno, Port Phillip. There are 47 vertebrate species with records from >15 years, but different filters should be discussed and decided.
...
## try only surveys that are starting with R and see whether RLS data has higher number of species per survey
## include lobsters and abalone and seastars and periwincles (turboungulatus), centrostephanus and heliocedaris

MeanVertLocation <- rls_bas  %>% 
  filter (( PHYLUM == "Chordata" & GENUS != "NA" & CLASS != "Ascidiacea") |  CLASS == "Cephalopoda" ) %>% 
 # filter ( PHYLUM == "Chordata" | CLASS  == "Cephalopoda") %>% 
...
ggplot(VertLocatOverlap, aes (x = reorder(TAXONOMIC_NAME, -meanLocations), y = meanLocations)) +
    geom_bar(stat =  "identity", fill= "grey") + 
        geom_errorbar(aes(ymin = meanLocations - sdLocations, ymax = meanLocations + sdLocations), width = 0.1) +
            xlab ("129 vertebrates species") + ylab("Mean number of locations per month/year (+/- SD)") + 
...
ggplot(VertLocatOverlapY, aes (x = reorder(TAXONOMIC_NAME, -meanLocations), y = meanLocations)) +
    geom_bar(stat =  "identity", fill= "grey") + 
        geom_errorbar(aes(ymin = meanLocations - sdLocations, ymax = meanLocations + sdLocations), width = 0.1) +
            xlab ("152 vertebrates species") + ylab("Mean number of locations per year (+/- SD)") + 
                theme(axis.text.x = element_blank()) 
...
mainInvSp <- rls_bas %>% 
  filter (PHYLUM != "Chordata") %>% #about 1/3 of records (54000) are for inverts
    group_by (TAXONOMIC_NAME) %>%
      summarise (abunMax = max(Abundance), abunMean = mean(Abundance), years = n_distinct(year)) %>%
        filter(years > 3, abunMax > 5, abunMean >2) 

MainInvList = mainInvSp[[1]] #58 species 
MainSpList = c(MainVertList, MainInvList)

temp = rls_bas$TAXONOMIC_NAME %in% MainSpList
tempInd = which(temp==TRUE)
rls_basMainSp <- rls_bas[tempInd,]


#HERE
rls_basMainSp5loc <- rls_

#First will do MDS on 5 main areas based on vertebrates only 




#Subsamples from rls_bas only records that include main vertebrates 
...
indL = rls_basCV$Location %in% LocVector
CommLocInd = which(indL==TRUE)              
rls_basRED <- rls_basCV[CommLocInd,]   #rls_bas dataset reduced to exclude rare species and rare locations


#now select from the rls_bas dataset only observations for the common species and locations 
...
rls_5 <- rls_bas %>%
    filter (rls_bas$loc =="Nine" | rls_bas$loc =="Tind" | rls_bas$loc =="Mari" | rls_bas$loc =="Bich" | rls_bas$loc =="Port") 
...
### Life history prediction parameters (Thorson et al. 2017)
...
The tables are somwhat overwhelming as they still have too many species/genera, but it is a bit of reduction. 

### Maximum and mean vertebrate size through time 

This is analysed by smaller and bigger species. Smaller species are those for which maximum recorded size is <50 cm
It looks to me that there are no clear trends, and the analysis is very crude and probably incorrect. I am only selecting species for which there are at least 10 years of data, and fitting a natural spline with n=2 (so a trendline)

```{r warning=FALSE, message=FALSE, warning=FALSE, eval=FALSE, echo=FALSE}

VertSize <- rls_bas %>% 
  filter (PHYLUM == "Chordata", GENUS != "NA") %>% 
    group_by (RecordedSpecies, GENUS, year) %>%
      summarise (abunMean = round(mean(Abundance),2), abunMax = max(Abundance), latt = mean(SiteLat), long = mean(SiteLong), dep_min = min(Depth), dep_max = max(Depth), sizeMean = mean(SizeClass), sizeMax = max(SizeClass)) %>%
           summarise (nyears = n_distinct(year)) %>% 
              filter (nyears > 9)

#This tells us that the are 45 vert species that have been recorded in more than 5 years and whichc have maximum abundance of at least 5 individuals observed at least once (remove rare species). 
#I select species that have been recorded in If maximum abundance criterion is not applied there are 112 species 

VertSizeList <- VertSize[[1]]  #82 vertebrate species that occur in 10 or more years

#now select only those observaton that include the 112 species 
tempvect = rls_bas$RecordedSpecies %in% VertSizeList
VertSizeInd = which(tempvect==TRUE)
rls_basVerSize <- rls_bas[VertSizeInd,] # this has 96 thousand records

#first plot species for which maximum size is <50
smallVerts <- rls_basVerSize %>%
    group_by (RecordedSpecies, GENUS, FAMILY, ORDER, CLASS) %>%
      summarise(sizeMax = max(SizeClass)) %>%
        filter (sizeMax < 50) %>% # filter out species that are never bigger than 50 cm
          summarise (nsp = n_distinct(RecordedSpecies))

SmallVertList = smallVerts[[1]] #37 species that are never bigger than 50 cm

temp = VertSizeList %in% SmallVertList
tempIn = which(temp==FALSE)
LargeVertList = VertSizeList[tempIn] #other 45 species that can get bigger than 50 cm

#datafile for plotting lengths
len_plot <- rls_basVerSize %>%
    group_by (RecordedSpecies, year) %>%
      summarise(sizeMean = mean(SizeClass), sizeMax = max(SizeClass))

years <- seq(from=1992,to=2017)

##plot maximum size
plot(x = years, y = years, ylab = "Max size", ylim = c(0, 50), type = "n")
 for (i in 1:length(SmallVertList)) { 

   plot_data <- len_plot %>%
  filter(RecordedSpecies == SmallVertList[i]) %>%
    arrange(year)

lines(spline(x = plot_data$year, y = plot_data$sizeMax, method='n', n=2), col='gray')
}


plot(x = years, y = years, ylab = "Max size", ylim = c(0, 150), type = "n")
 for (i in 1:length(LargeVertList)) { 

   plot_data <- len_plot %>%
  filter(RecordedSpecies == LargeVertList[i]) 

lines(spline(x = plot_data$year, y = plot_data$sizeMax, method='n', n=2), col='gray')
}

#Plot average size
plot(x = years, y = years, ylab = "Average size", ylim = c(0, 40), type = "n")
 for (i in 1:length(SmallVertList)) { 
...
lines(spline(x = plot_data$year, y = plot_data$sizeMean, method='n', n=2), col='gray')
}


plot(x = years, y = years, ylab = "Average size", ylim = c(0, 150), type = "n")
 for (i in 1:length(LargeVertList)) { 
...
#count the number of observationsn of each species in ecah location   

VertByLoc_To2000 <- rls_bas %>% 
       filter(year < 2000) %>% 
        filter (PHYLUM == "Chordata") %>%
          filter (Abundance > 1) %>%
         group_by(Location) %>% 
           summarise (nsp = n_distinct(SpeciesID))

vert_plot <- rls_bas %>% 
  filter(year < 2000, PHYLUM == "Chordata", Abundance > 1) %>% 
          arrange(SiteLat) %>%
           group_by(Location) %>% 
           summarise (nsp = n_distinct(SpeciesID)) 

locAll_more1 <- rls_bas %>% 
  filter(Abundance > 1) %>% 
           group_by(Location) %>% 
           summarise (latt = mean(SiteLat), long = mean(SiteLong), nsp = n_distinct(SpeciesID), year = n_distinct(year)) 
...
MarIslRes <- rls_bas %>%
  filter(Location == "Maria Island Reserve", PHYLUM == "Chordata") %>%
    group_by(SPECIES_NAME, year) %>% 
        #summarise_each (funs(min, max, mean), Abundance, SizeClass) 
        summarise (mean(Abundance), mean(BioMass), mean(SizeClass), min(SizeClass), max(SizeClass))
...
#I only want to choose species that occur in more than 2 years and that have more than 3 (5?) observations per year
commSpecMIR <- MarIslResN %>%
  filter(n > 4) %>% #remove cases where abundance in the year was < 5
   group_by(SPECIES_NAME) %>%
    summarise (n = n()) %>%
      filter (n > 2) #remove cases where species was observed in fewer than 3 years 


#filter out rare species 
commVertAll <- rls_bas %>%
  filter(PHYLUM == "Chordata") %>%
    group_by(Location, SPECIES_NAME, year) %>% 
        summarise(n = n()) %>%  # number of observations per location per species per year
          filter (n >4) %>% #remove species which have been observed <5 times in a given year/location
            group_by(SPECIES_NAME) %>%
              summarise(n = n()) %>%
                filter (n > 3) #remove cases where species was observed in fewer than 3 years: 74 species, and if fewers than 4 years then we have 66 species 

VertVector <- commVertAll[[1]] ## list of 66 vert species to be used in MDS
...
indV = rls_bas$SPECIES_NAME %in% VertVector
CommVertInd = which(indV==TRUE)
rls_basCV <- rls_bas[CommVertInd,]
...
#select only vertebrates first 
dataforMDS <- rls_basRED %>%
    group_by(locyear, SPECIES_NAME) %>%
      summarise (abun = mean(Abundance))

#here I tried to treat locations and year independently, but there does not seem to be a clear trend in years

#create a location/species abundance matrix and convert it (in a clumsy way) from list to a numeric matrix

dataMDS <- spread(dataforMDS, key = locyear, value = abun, fill = 0)

sdatet <- matrix(unlist(sdate),ncol=3,byrow=TRUE) #turn the list into matrix

dataMDSm <- matrix(data = NA, nrow = dim(dataMDS)[1], ncol = dim(dataMDS)[2])

colnames(dataMDSm) <- colnames(dataMDS)
rownames(dataMDSm) <- dataMDS[[1]]

for (i in 1:dim(dataMDS)[2]) {
    dataMDSm[,i] <- c(as.double(dataMDS[[i]]))
}

dataMDSm <- round(dataMDSm[,-1],3)
dataMDSmt <- t(dataMDSm)

##now try to do the MDS
library(vegan)

bas_dist = vegdist(dataMDSmt)

mdstest = metaMDS(dataMDSmt, k=2) #no convergence 
mdstest = metaMDS(dataMDSmt, distance="bray",k=3,trymax=200)
...
ordiplot(mdstest,type="n")
orditorp(mdstest,display="species",col="red",air=0.01)
orditorp(mdstest,display="sites",cex=0.5,air=0.01)


set.seed(2)
test = metaMDS(dataMDSm, k = 2)


Y9295 <- rls_bas %>%
    filter(PHYLUM == "Chordata", year < 1996) %>% 
      group_by (SpeciesID, Location) %>%
        summarise ( ab = mean(Abundance))

Y9295d <- spread(data = Y9295, key = Location, value = ab, fill= 0)
mode(Y9295d)


colnames(dataNum) <- Y9295d[1,]
spnames = as.vector(Y9295d[,1])
rownames(dataNum) <- Y9295d[,1]


mdsDat<-metaMDS(Y9295d,distance = "bray", k = 2, trymax = 50)
...
#shane's examples
df_plot <- df_long %>% 
  filter(m1 == 8, rb == 0.6, ra == 6, RSmax == 1.3,
    Fm == levs[3], Prediction == "Opt_w") %>% 
  arrange(s1)

plot(x = df_plot$s1, y = df_plot$Value, type = "o", pch = 21, col = "red", bg = "red",
  xaxs = "r", yaxs = "i", xlab = "", ylab = "", xlim = c(4.5, 9.5), ylim = c(0.0, 1.05))

rls_bas %>% group_by(Location) %>% summarise(rls_bas, n_distinct(rls_bas$SpeciesID))



n_distinct(rls_bas$SpeciesID, group_by= rls_bas$Location)



#other potentially useful functions that were not used 
date <- strtrim(rls_bas$SurveyDate, 10)
n_distinct(rls_bas, Location) #
